...
- The ID token is decoded **without signature verification** in the browser (`IdTokenClaims::parse_unverified` in `src/modules/oidc/tokens.rs`) purely to read display claims. This is an accepted SPA pattern: the token arrives over TLS directly from the issuer, and the backend independently re-validates the access token on every API call, so the browser never trusts the ID token for authorization.
