//! mokosh-contact-login prompt 005: contact-plane password reset.
//!
//! Mounted at `/portal/{slug}/reset-password?token=...`. Public route,
//! token arrives as a component prop. Mirrors the shape of
//! `set_password.rs` (same 204-no-body response, same "sign in below"
//! post-success posture, same rationale for not auto-logging in).
...
pub fn ContactResetPasswordPage(slug: String, token: String) -> Element {
    ...
    let heading = match company_name.as_deref() {
        Some(name) => format!("Reset your password for {name}"),
