...
/// Primary path of [`WalletIcon`] (Heroicons `wallet`).
pub const WALLET_PATH: &str = "M21 12a2.25 2.25 0 0 0-2.25-2.25H15a3 3 0 1 1-6 0H5.25A2.25 2.25 0 0 0 3 12m18 0v6a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 18v-6m18 0V9M3 12V9m18 0a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 9m18 0V6a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 6v3";

/// A wallet. MAPPS-782: the portal's Payment Methods entry, distinct from the
/// staff Payments row (`CreditCardIcon`) because every sidebar row carries its
/// own glyph (MAPPS-359), and a customer's saved cards are theirs to hold
/// rather than a ledger of money received.
#[component]
pub fn WalletIcon(#[props(default)] size: IconSize, #[props(default)] class: String) -> Element {
    let size_class = size.class();
    let class = format!("{} {}", size_class, class);
...
#[component]
pub fn SunIcon(#[props(default)] size: IconSize, #[props(default)] class: String) -> Element {
    let size_class = size.class();
    let class = format!("{} {}", size_class, class);
    rsx! {
        svg {
...
/// PMS-729 phase 2 §6 slice 3: dark-mode indicator for the portal
/// theme toggle. Heroicons `moon` outline.
#[component]
pub fn MoonIcon(#[props(default)] size: IconSize, #[props(default)] class: String) -> Element {
    let size_class = size.class();
    let class = format!("{} {}", size_class, class);
    rsx! {
        svg {
