        ...
        div { class: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6",
            for _ in 0..props.count {
                div {
                    ...
                    // title + subtitle
                    div { class: "h-5 w-2/3 bg-surface-2 rounded animate-pulse" }
                    div { class: "h-4 w-1/2 bg-surface-2 rounded animate-pulse" }
                    // a body line
                    div { class: "h-3 w-full bg-surface-2 rounded animate-pulse" }
                    // a progress-bar-shaped block (project cards show a budget bar)
                    div { class: "h-2 w-full bg-surface-2 rounded-full animate-pulse" }
                }
            }
        }
    }
