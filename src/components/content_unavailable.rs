...
//! renders an explicit, honest state instead, and because the page keeps
//! its [`AppLayout`] wrapper, the sidebar nav, top bar, and outage banner
...
//!
//! Mirrors the [`crate::components::PermissionRequired`] empty-state
//! idiom (AppLayout + PageHeader + centered Card) so the two read as one
//! family.

use dioxus::prelude::*;

use crate::components::{AppLayout, Card, IconSize, PageHeader, ServerIcon};
use crate::Route;
